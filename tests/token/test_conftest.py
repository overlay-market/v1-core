...
def test_roles(
    ...
    assert (
        token.getRoleAdmin(token.DEFAULT_ADMIN_ROLE())
        == token.DEFAULT_ADMIN_ROLE()
    )
    assert (
        token.getRoleAdmin(minter_role) == token.DEFAULT_ADMIN_ROLE()
    )
    assert (
        token.getRoleAdmin(burner_role) == token.DEFAULT_ADMIN_ROLE()
    )
    assert (
        token.getRoleAdmin(governor_role)
        == token.DEFAULT_ADMIN_ROLE()
    )
