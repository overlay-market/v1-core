...
def test_get_quote_at_tick_for_daiweth_without_reserve(
    ...
    expect_sign = (
        -1 if base_token.address > quote_token.address else 1
    )
    expect_quote_amount = int(
        base_amount * 1.0001 ** (expect_sign * tick)
    )
...
def test_get_reserve_in_weth_for_daiweth(weth, quanto_feed):
    ...
    expect_reserve = (
        expect_x if quanto_feed.marketToken0 == weth else expect_y
    )
...
def test_get_reserve_in_weth_for_uniweth(weth, inverse_feed):
    ...
    expect_reserve = (
        expect_x if inverse_feed.marketToken0 == weth else expect_y
    )
...
def test_get_reserve_in_ovl(uni, weth, quanto_feed):
    ...
    sign = (
        -1
        if base_token_ovlweth.address > quote_token_ovlweth.address
        else 1
    )

    # num of WETH / num of UNI
    price_ovlweth = int(
        base_amount_ovlweth * 1.0001 ** (sign * tick_ovlweth)
    )

    expect_reserve_in_ovl = int(
        expect_reserve * base_amount_ovlweth / price_ovlweth
    )
    actual_reserve_in_ovl = quanto_feed.getReserveInOvl(
        tick_market, liquidity, tick_ovlweth
    )
