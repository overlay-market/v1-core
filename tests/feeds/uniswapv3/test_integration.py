...
def test_consult_for_daiweth(pool_daiweth_30bps, quanto_feed):
    ...
    tick_cums, secs_per_liq_cums = pool_daiweth_30bps.observe(
        seconds_agos
    )
    actual_avg_ticks, actual_avg_liqs = quanto_feed.consult(
    ...
    for i in range(len(windows)):
        expect_avg_tick = int(
            (tick_cums[now_idxs[i]] - tick_cums[i]) / windows[i]
        )
        expect_avg_liq = int(
            windows[i]
            * ((1 << 160) - 1)
        ...
        assert (
            approx(expect_avg_tick, rel=1e-4) == actual_avg_ticks[i]
        )
        assert approx(expect_avg_liq) == actual_avg_liqs[i]


def test_consult_for_daiweth_without_reserve(
    ...
    actual_avg_ticks = quanto_feed_without_reserve.consult(
        pool_daiweth_30bps, seconds_agos, windows, now_idxs
    )
...
def test_consult_for_uniweth(pool_uniweth_30bps, inverse_feed):
    ...
    tick_cums, secs_per_liq_cums = pool_uniweth_30bps.observe(
        seconds_agos
    )
