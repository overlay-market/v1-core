from brownie import (
    ...
    OverlayV1NoReserveUniswapV3Feed,
)
...
def test_deploy_feed_creates_quanto_feed_without_reserve(
    ...
    tx = factory_without_reserve.deployFeed(
        market_base_token,
        market_quote_token,
        market_fee,
        market_base_amount,
    ...
    assert (
        factory_without_reserve.getFeed(
            market_pool, market_base_token, market_base_amount
        )
        == actual_feed
    )
...
def test_deploy_feed_creates_quanto_feed(
    factory,
    pool_uniweth_30bps,
    uni,
    pool_daiweth_30bps,
...
def test_deploy_feed_creates_inverse_feed(
    factory, pool_uniweth_30bps, uni, weth, alice
):
...
def test_deploy_feed_reverts_when_market_pool_not_exists(
    factory,
    alice,
    bob,
    pool_daiweth_30bps,
...
def test_deploy_feed_reverts_when_ovlx_pool_not_exists(
    factory,
    alice,
    bob,
    pool_daiweth_30bps,
...
def test_deploy_no_reserve_feed_reverts_when_feed_already_exists(
    factory_without_reserve, dai, weth, pool_daiweth_30bps, alice
):
    ...
    feed = factory_without_reserve.getFeed(
        market_pool, market_base_token, market_base_amount
    )
...
def test_deploy_feed_reverts_when_feed_already_exists(
    factory,
    uni,
    dai,
    weth,
