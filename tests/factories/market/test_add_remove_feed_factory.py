...
def test_add_feed_factory_adds_factory(factory, rando, gov):
    ...
    expect_event = OrderedDict(
        {"user": gov.address, "feedFactory": rando}
    )
    actual_event = tx.events["FeedFactoryAdded"]
    assert actual_event == expect_event


def test_add_feed_factory_reverts_when_not_gov(
    factory, charlie, alice
):
...
def test_add_feed_factory_reverts_when_factory_already_exists(
    factory, feed_factory, gov
):
...
def test_remove_feed_factory_removes_feed_factory(
    factory, rando, gov
):
    assert factory.isFeedFactory(rando) is True
...
def test_remove_feed_factory_reverts_when_not_gov(
    factory, rando, gov
):
...
def test_remove_feed_factory_reverts_when_not_feed_factory(
    factory, alice, gov
):
