...
def test_updated_fraction_remaining(
    position, fraction_remaining, fraction_removed
):
    notional = 10000000000000000000  # 10
    debt = 2000000000000000000  # 2
    is_long = True
    ...
    updated_fraction_remaining_256 = (
        fraction_remaining_256
        * (Decimal(1e18) - fraction_removed_256)
        / Decimal(1e18)
    )
    ...
    actual = position.updatedFractionRemaining(
        pos, fraction_removed_256
    )
    assert expect == actual
...
def test_updated_fraction_remaining_reverts_when_gt_max(position):
    ...
    fraction_removed = (
        1000000000000000001  # 1 greater than ONE (18 decimals)
    )
