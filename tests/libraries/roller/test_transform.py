...
@given(
    accumulator_last=strategy(
        "decimal",
        min_value="-10000000.000",
        max_value="10000000.000",
        places=3,
    ),
    window_last=strategy(
        "uint256", min_value="10000", max_value="7776000"
    ),
    value=strategy(
        "decimal",
        min_value="-10000000.000",
...
def test_transform(
    roller, accumulator_last, window_last, value, window, dt
):
    """
    ...
    accumulator_last_decayed = accumulator_last * (
        1 - dt / window_last
    )
    ...
    w2 = abs(value)
    expect_window = (
        int(((window_last - dt) * w1 + window * w2) / (w1 + w2))
        if expect_value != 0
        else window
    )
...
def test_transform_when_timestamp_32_wraps(roller):
    ...
    w2 = abs(value)
    expect_window = int(
        (w1 * (window_last - dt) + w2 * window) / (w1 + w2)
    )
