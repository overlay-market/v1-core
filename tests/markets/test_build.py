...
@given(
    notional=strategy(
        "decimal", min_value="0.001", max_value="80000", places=3
    ),
    leverage=strategy(
        "decimal", min_value="1.0", max_value="5.0", places=3
    ),
    is_long=strategy("bool"),
)
def test_build_creates_position(
    market, feed, ovl, alice, notional, leverage, is_long
):
    ...
    expect_oi_shares = int(
        oi * Decimal(1e18)
    )  # same since first position
    ...
    assert int(actual_notional_initial) == approx(
        expect_notional_initial
    )
...
def test_build_registers_volume(
    market, feed, ovl, alice, notional, leverage, is_long
):
    # calculate expected pos info data
    idx_trade = RiskParameter.TRADING_FEE_RATE.value
...
def test_build_executes_transfers(
    market, factory, ovl, alice, notional, leverage, is_long
):
    # calculate expected pos info data
    idx_trade = RiskParameter.TRADING_FEE_RATE.value
    trading_fee_rate = Decimal(market.params(idx_trade) / 1e18)
...
def test_build_transfers_collateral_to_market(
    market, ovl, alice, notional, leverage, is_long
):
    # calculate expected pos info data
    idx_trade = RiskParameter.TRADING_FEE_RATE.value
    trading_fee_rate = Decimal(market.params(idx_trade) / 1e18)
    ...
    expect_collateral_in = int(
        (collateral + trade_fee) * Decimal(1e18)
    )
...
def test_build_transfers_trading_fees(
    market, factory, ovl, alice, notional, leverage, is_long
):
    # calculate expected pos info data
    idx_trade = RiskParameter.TRADING_FEE_RATE.value
...
def test_build_reverts_when_leverage_less_than_one(
    market, ovl, alice
):
    # NOTE: current position id is zero given isolation fixture
    expect_pos_id = 0
...
def test_build_reverts_when_leverage_greater_than_cap(
    market, ovl, alice
):
    # NOTE: current position id is zero given isolation fixture
    expect_pos_id = 0
...
def test_build_reverts_when_collateral_less_than_min(
    market, ovl, alice, leverage, is_long
):
    # NOTE: current position id is zero given isolation fixture
    expect_pos_id = 0
...
def test_build_reverts_when_oi_greater_than_cap(
    market, ovl, alice, is_long
):
    # NOTE: current position id is zero given isolation fixture
    expect_pos_id = 0
...
def test_build_reverts_when_oi_greater_than_cap_w_circuit_breaker(
    ...
    factory.setRiskParam(
        mock_feed, idx_mint, mint_target, {"from": gov}
    )
    ...
    cap_notional = mock_market.params(
        RiskParameter.CAP_NOTIONAL.value
    )
    ...
    notional_initial = (
        Decimal(cap_oi_circuited) * price / Decimal(1e36)
    )
    leverage = Decimal(1)
    ...
    # input values for build
    input_collateral = int(
        collateral * Decimal(1 + tol) * Decimal(1e18)
    )
    input_leverage = int(leverage * Decimal(1e18))
    input_is_long = True
...
@given(is_long=strategy("bool"))
def test_build_reverts_when_liquidatable(
    mock_market, feed, ovl, alice, is_long
):
    ...
    if is_long:
        entry_price = mid_price * (
            1
            - maintenance_fraction / (1 - liq_fee_rate)
            + Decimal(1) / leverage
        )
    else:
        entry_price = mid_price * (
    ...
    if is_long:
        volume = (
            Decimal(log(entry_price / mid_price)) - delta
        ) / lmbda
    else:
        volume = (
            Decimal(-1)
...
def test_build_reverts_when_oi_zero(
    mock_market, mock_feed, ovl, alice, bob
):
    # NOTE: current position id is zero given isolation fixture
    expect_pos_id = 0
...
def test_build_reverts_when_has_shutdown(
...
def test_multiple_build_creates_multiple_positions(
    market, factory, ovl, feed, alice, bob
):
    ...
    for i in range(n):
        ...
        expect_oi_alice = int(
            Decimal(expect_notional_alice)
            * Decimal(1e18)
            / Decimal(mid_price)
        )
        expect_oi_shares_alice = (
            expect_oi_alice
            if expect_oi_long == 0
            else int(
                (Decimal(expect_oi_alice) / Decimal(expect_oi_long))
        ...
        (
            actual_notional_alice,
            actual_debt_alice,
            actual_mid_tick_alice,
            actual_entry_tick_alice,
            actual_is_long_alice,
            actual_liquidated_alice,
            actual_oi_shares_alice,
            actual_fraction_remaining_alice,
        ) = actual_pos_alice

        actual_mid_price_alice = tick_to_price(actual_mid_tick_alice)
        actual_oi_alice = int(
            Decimal(actual_notional_alice)
            * Decimal(1e18)
            / Decimal(actual_mid_price_alice)
        )

        assert int(actual_notional_alice) == approx(
            expect_notional_alice
        )
        ...
        assert int(actual_oi_shares_alice) == approx(
            expect_oi_shares_alice
        )
        # NOTE: use 1e-4 since mid from tick has 4 decimal precision
        assert int(actual_oi_alice) == approx(
            expect_oi_alice, rel=1.25e-4
        )
        ...
        assert int(actual_oi_long_shares) == approx(
            expect_oi_long_shares
        )
        ...
        expect_oi_bob = int(
            Decimal(expect_notional_bob)
            * Decimal(1e18)
            / Decimal(mid_price)
        )
        expect_oi_shares_bob = (
            expect_oi_bob
            if expect_oi_short == 0
            else int(
                (Decimal(expect_oi_bob) / Decimal(expect_oi_short))
        ...
        (
            ...
            actual_debt_bob,
            actual_mid_tick_bob,
            actual_entry_tick_bob,
            actual_is_long_bob,
            actual_liquidated_bob,
            actual_oi_shares_bob,
            actual_fraction_remaining_bob,
        ) = actual_pos_bob

        actual_mid_price_bob = tick_to_price(actual_mid_tick_bob)
        actual_oi_bob = int(
            Decimal(actual_notional_bob)
            * Decimal(1e18)
            / Decimal(actual_mid_price_bob)
        )
        ...
        assert int(actual_oi_shares_bob) == approx(
            expect_oi_shares_bob
        )
        # NOTE: use 1e-4 since mid from tick has 4 decimal precision
        assert int(actual_oi_bob) == approx(
            expect_oi_bob, rel=1.25e-4
        )
        ...
        assert int(actual_mid_tick_bob) == approx(
            expect_mid_tick_bob, abs=1
        )
        ...
        assert int(actual_oi_short_shares) == approx(
            expect_oi_short_shares
        )
