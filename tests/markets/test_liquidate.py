...
def test_liquidate_updates_position(
    mock_market, mock_feed, alice, rando, ovl, is_long
):
    ...
    liq_collateral = (
        liq_notional * (liq_oi / liq_oi_initial) - liq_debt
    )
    ...
    tx = mock_market.liquidate(
        input_owner, input_pos_id, {"from": rando}
    )
    ...
    maintenance_burn = Decimal(
        mock_market.params(idx_mmbr)
    ) / Decimal(1e18)
...
def test_liquidate_updates_market(
    mock_market, mock_feed, alice, rando, ovl
):
    # position build attributes
    notional_initial = Decimal(1000)
    leverage = Decimal(1.5)
...
@given(is_long=strategy("bool"))
def test_liquidate_registers_zero_volume(
    mock_market, mock_feed, alice, rando, ovl, is_long
):
    # position build attributes
    notional_initial = Decimal(1000)
...
@given(is_long=strategy("bool"))
def test_liquidate_registers_mint_or_burn(
    mock_market, mock_feed, alice, rando, ovl, is_long
):
    # position build attributes
    notional_initial = Decimal(1000)
...
@given(is_long=strategy("bool"))
def test_liquidate_executes_transfers(
    mock_market, mock_feed, alice, rando, factory, ovl, is_long
):
    # position build attributes
    notional_initial = Decimal(1000)
    leverage = Decimal(1.5)
...
@given(is_long=strategy("bool"))
def test_liquidate_transfers_fee_to_liquidator(
    mock_market, mock_feed, alice, rando, ovl, is_long
):
    # position build attributes
    notional_initial = Decimal(1000)
...
# TODO: check for correctness again
@given(is_long=strategy("bool"))
def test_liquidate_transfers_remaining_margin(
    mock_market, mock_feed, alice, factory, rando, ovl, is_long
):
    # position build attributes
    notional_initial = Decimal(1000)
    leverage = Decimal(1.5)
...
def test_liquidate_floors_value_to_zero_when_position_underwater(
    ...
    price_multiplier = Decimal(
        0.700
    )  # close to underwater but not there yet
...
def test_liquidate_reverts_when_not_position_owner(
    mock_market, mock_feed, alice, bob, rando, ovl
):
    # position build attributes
    notional_initial = Decimal(1000)
    leverage = Decimal(1.5)
...
def test_liquidate_reverts_when_position_not_exists(
    mock_market, alice, rando, ovl
):
    pos_id = 100
...
def test_liquidate_reverts_when_position_liquidated(
    mock_market, mock_feed, alice, rando, ovl
):
    # position build attributes
    notional_initial = Decimal(1000)
...
def test_liquidate_reverts_when_position_not_liquidatable(
    mock_market, mock_feed, alice, rando, ovl
):
    # position build attributes
    notional_initial = Decimal(1000)
...
def test_liquidate_reverts_when_has_shutdown(
    factory, mock_feed, mock_market, ovl, alice, guardian, rando
):
    # build inputs
