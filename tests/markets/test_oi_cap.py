...
def test_cap_notional_back_run_bound(market, feed):
    ...
    # NOTE: assumes using UniswapV3 feed with hasReserve = true
    average_block_time = market.params(
        RiskParameter.AVERAGE_BLOCK_TIME.value
    )
...
def test_cap_notional_adjusted_for_bounds_when_no_reserve(
    market, feed
):
    # Test cap notional adjustments is min of all bounds and circuit breaker
    idx = RiskParameter.CAP_NOTIONAL.value
    ...
    data = (
        ...
        600,
        3600,
        2729583770051358617413,
        2739701430255362520176,
        2729583770051358617413,
        1909229154186640322863637,
        False,
    )  # has_reserve = False
...
@given(
    minted=strategy(
        "decimal", min_value="66670", max_value="133340", places=1
    )
)
...
@given(
    minted=strategy(
        "decimal", min_value="-133340", max_value="66670", places=1
    )
)
def test_cap_notional_circuit_breaker_when_minted_less_than_target(
    market, minted
):
    idx = RiskParameter.CAP_NOTIONAL.value
    cap_notional = market.params(idx)
...
@given(
    minted=strategy(
        "decimal", min_value="133340", max_value="266680", places=1
    )
)
def test_cap_notional_circuit_breaker_when_mint_greater_than_2x_target(
    market, minted
):
    idx = RiskParameter.CAP_NOTIONAL.value
    cap_notional = market.params(idx)
...
def test_cap_oi_adjusted_for_circuit_breaker(market, feed):
    ...
    cap_oi = int(
        Decimal(cap_notional) * Decimal(1e18) / Decimal(mid_price)
    )
...
def test_cap_oi_adjusted_for_circuit_breaker_after_mint(
    ...
    cap_oi = int(
        Decimal(cap_notional) * Decimal(1e18) / Decimal(price)
    )
    snapshot = mock_market.snapshotMinted()

    # calculate cap oi adjusted for circuit breaker
    cap_oi_circuit_breaker = mock_market.circuitBreaker(
        snapshot, cap_oi
    )
