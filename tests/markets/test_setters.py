...
def test_set_risk_param_pays_funding(
    market, feed, factory, ovl, alice
):
    # risk params
    expect_params = [
        2220000000000,  # expect_k
    ...
    for i in range(len(RiskParameter)):
        ...
        assert (
            actual_timestamp_update_last
            == expect_timestamp_update_last
        )
        assert (
            actual_timestamp_update_last
            != prior_timestamp_update_last
        )
...
def test_set_delta_reverts_when_max_lev_liquidatable(market, factory):
    ...
    cap_leverage = Decimal(market.params(idx_cap_leverage)) / Decimal(
        1e18
    )
...
def test_set_cap_leverage_reverts_when_max_lev_liquidatable(
    market, factory
):
    # idx for delta, cap_leverage, maintenance, liquidation fee rate
    idx_delta = RiskParameter.DELTA.value
    ...
    input_cap_leverage = int(
        max_cap_leverage * Decimal(1e18) * Decimal(1 + tol)
    )
    with reverts("OVLV1: max lev immediately liquidatable"):
        market.setRiskParam(
            idx_cap_leverage, input_cap_leverage, {"from": factory}
        )
...
def test_set_price_drift_upper_limit_reverts_when_exceeds_max_exp(
    ...
    idx_price_drift_upper_limit = (
        RiskParameter.PRICE_DRIFT_UPPER_LIMIT.value
    )

    _, _, macro_window, _, _, _, _, _ = feed.latest()
    max_exp = 20000000000000000000
    max_price_drift_upper_limit = (
        int(Decimal(max_exp) / Decimal(macro_window)) + 1
    )
