...
def test_oi_after_funding(market, oi_long, oi_short, dt):
    ...
    (
        actual_oi_overweight,
        actual_oi_underweight,
    ) = market.oiAfterFunding(oi_overweight, oi_underweight, dt)
...
@given(
    oi_overweight=strategy(
        "decimal", min_value="0.001", max_value="800000", places=3
    ),
    dt=strategy("uint256", min_value="0", max_value="2592000"),
)
def test_oi_after_funding_when_underweight_is_zero(
    market, oi_overweight, dt
):
...
def test_oi_after_funding_when_longs_and_shorts_are_zero(market):
    ...
    (
        actual_oi_overweight,
        actual_oi_underweight,
    ) = market.oiAfterFunding(oi_long, oi_short, dt)
...
def test_oi_after_funding_when_underweight_is_zero_dt_infinite(
    market,
):
