...
/// @notice Q64.96 numbers. Supports / prices between 2**-128 and 2**128
library TickMath {
    ...
    /// two assets (token1/token0) / at the given tick
    function getSqrtRatioAtTick(int24 tick) internal pure returns (uint160 sqrtPriceX96) {
        uint256 absTick = tick < 0 ? uint256(-int256(tick)) : uint256(int256(tick));
        // XXX: Cast MAX_TICK to uint24 prior to require check w absTick
        ...
        // input constraint we round up in the division so getTickAtSqrtRatio of the output price
    ...
    /// @dev getRatioAtTick may / ever return.
