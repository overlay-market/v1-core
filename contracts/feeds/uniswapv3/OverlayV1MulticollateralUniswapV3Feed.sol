...
contract OverlayV1MulticollateralUniswapV3Feed is IOverlayV1UniswapV3Feed, OverlayV1Feed {
    uint128 internal constant ONE = 1e18; // 18 decimal places for ovl

    mapping public collateralPools(address => address);
    ...
    /// @param _collateral the address of the erc20 collateral being used
    function _fetch(address _collateral) internal view virtual override returns (Oracle.Data memory) {
        ...
        //  0: marketPriceOneMacroWindowAgo: prices[0]
        //  1: marketPriceOverMacroWindow: prices[1]
        //  2: marketPriceOverMicroWindow: prices[2]
        // window: [now - macroWindow * 2, now - macroWindow]
        uint256 marketPriceOneMacroWindowAgo = getQuoteAtTick(
        ...
        uint256 marketPriceOverMacroWindow = getQuoteAtTick(
        ...
        uint256 marketPriceOverMicroWindow = getQuoteAtTick(
        ...
        if (marketPool == ovlXPool) {
            ...
            ( uint32[] memory secondsAgosOvlX,
              ...
              uint256[] memory nowIdxsOvlX) = _inputsToConsultOvlXPool();
...

        {
          ...
          ( uint32[] memory secondsAgosCollateral,
            uint32[] memory windowsCollateral,
            uint256[] memory nowIdxsCollateral ) = _inputsToConsultCollateralPool(microWindow, macroWindow);

          ( int24[] memory arithmeticMeanTicksCollateral, ) = consult(
            collateralPools[_collateral],
            secondsAgosCollateral,
            windowsCollateral,
            nowIdxsCollateral );
        ...
        Oracle.Data({
          ...
          microWindow
          macroWinow
          marketPriceOverMicro
          marketPriceOverMacro
          marketPriceOneMacroAgo
          reserveOverMicro
          hasReserve
        })
        ...
        return
            Oracle.Data({
                ...
                marketPriceOverMicroWindow: marketPriceOverMicroWindow,
                marketPriceOverMacroWindow: marketPriceOverMacroWindow,
                marketPriceOneMacroWindowAgo: marketPriceOneMacroWindowAgo,
                collateralPriceAtMidMarket: collateralPriceAtMidMarket,
        ...
        )
    {
        ...
        // in terms of prices, will use for indexes
        //  0: marketPriceOneMacroWindowAgo
        //  1: marketPriceOverMacroWindow
        //  2: marketPriceOverMicroWindow
    ...
    /// @dev returns input params needed for call to ovlXPool consult
    function _inputsToConsultCollateralPool(uint256 _microWindow, uint256 _macroWindow)
        private
        pure
        returns (
            uint32[] memory,
        ...
        )
    {
        ...
        uint256[] memory nowIdxs = new uint256[](2);
